...
object Cluster extends Controller {
  ...
  def updateCluster(c: String) = Action.async { implicit request =>
    kafkaManager.getClusterConfig(c).map { errorOrClusterConfig =>
      Ok(views.html.cluster.updateCluster(c,errorOrClusterConfig.map { cc =>
        updateForm.fill(ClusterOperation.apply(Update.toString, cc.name, cc.version.toString, cc.curatorConfig.zkConnect, cc.curatorConfig.zkMaxRetry, cc.jmxEnabled, cc.displaySizeEnabled))
      }))
    }
  }
  ...
  def handleAddCluster = Action.async { implicit request =>
    clusterConfigForm.bindFromRequest.fold(
      ...
      clusterConfig => {
        kafkaManager.addCluster(clusterConfig.name, clusterConfig.version.toString, clusterConfig.curatorConfig.zkConnect, clusterConfig.jmxEnabled, clusterConfig.displaySizeEnabled).map { errorOrSuccess =>
