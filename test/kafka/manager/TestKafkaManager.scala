...
class TestKafkaManager extends CuratorAwareTest with BaseTest {
  ...
  test("update cluster zkhost") {
    val future = kafkaManager.updateCluster("dev","1.1.0",testServer.getConnectString, jmxEnabled = false, pollConsumers = true, filterConsumers = true, jmxUser = None, jmxSsl = false, jmxPass = None, tuning = Option(defaultTuning), securityProtocol = "PLAINTEXT", saslMechanism = "PLAIN", jaasConfig = None)
    val result = Await.result(future,duration)
    assert(result.isRight === true)
  ...
  test("update cluster version") {
    val future = kafkaManager.updateCluster("dev","0.8.1.1",testServer.getConnectString, jmxEnabled = false, pollConsumers = true, filterConsumers = true, jmxUser = None, jmxPass = None, jmxSsl = false, tuning = Option(defaultTuning), securityProtocol = "PLAINTEXT", saslMechanism = "PLAIN", jaasConfig = None)
    val result = Await.result(future,duration)
    assert(result.isRight === true)
    Thread.sleep(2000)
