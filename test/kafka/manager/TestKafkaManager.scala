...
class TestKafkaManager extends CuratorAwareTest with BaseTest {
  ...
  test("add cluster") {
    val future = kafkaManager.addCluster("dev","2.5.1",kafkaServerZkPath, jmxEnabled = false, pollConsumers = true, filterConsumers = true, jmxUser = None, jmxPass = None, jmxSsl = false, tuning = Option(kafkaManager.defaultTuning), securityProtocol="PLAINTEXT", saslMechanism = None, jaasConfig = None)
    val result = Await.result(future,duration)
    assert(result.isRight === true)
    Thread.sleep(2000)
  }
  ...
  test("update cluster zkhost") {
    val future = kafkaManager.updateCluster("dev","2.5.1",testServer.getConnectString, jmxEnabled = false, pollConsumers = true, filterConsumers = true, jmxUser = None, jmxSsl = false, jmxPass = None, tuning = Option(defaultTuning), securityProtocol = "PLAINTEXT", saslMechanism = None, jaasConfig = None)
    val result = Await.result(future,duration)
    assert(result.isRight === true)
  ...
  test("update cluster logkafka enabled and activeOffsetCache enabled") {
    val future = kafkaManager.updateCluster("dev","2.5.1",testServer.getConnectString, jmxEnabled = false, pollConsumers = true, filterConsumers = true, logkafkaEnabled = true, activeOffsetCacheEnabled = true, jmxUser = None, jmxPass = None, jmxSsl = false, tuning = Option(defaultTuning), securityProtocol = "PLAINTEXT", saslMechanism = None, jaasConfig = None)
    val result = Await.result(future,duration)
    assert(result.isRight === true)
